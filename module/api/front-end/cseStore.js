...
export default function({speciesData, form}) {
    const store = new Store({
        actions: {
            async init(context) {
                if(foundry.utils.isEmpty(context.state.speciesData ?? {})) return await context.dispatch("initNewMon");
            ...
            async changeSpecies(context, species) {
                const speciesData = species === "" ? foundry.utils.duplicate(BlankPTUSpecies) : game.ptu.utils.species.get(species);
        ...
        mutations: {
            ...
            async changeTyping(state, {index, targetType}) {
                const newTypings = foundry.utils.duplicate(state.typings);
            ...
            async changeCapability(state, {index, newName}) {
                const newCapabilities = foundry.utils.duplicate(state.otherCapabilities);
            ...
            async changeMove(state, {index, newName, newLevel, evo, egg, tutor, tm}) {
                const newMoves = foundry.utils.duplicate(state.moves);
            ...
            async changeAbility(state, {index, newName, newTier, newIndex}) {
                const newAbilities = foundry.utils.duplicate(state.abilities);
