...
export class PTUActor extends Actor {
  ...
  prepareData() {
    ...
    this.origins = foundry.utils.mergeObject({
      system: {
        levelUpPoints: [
  ...
  applyActiveEffects(doBaseData = true) {
    ...
    for (let change of changes) {
      ...
      if (result !== null) {
        overrides = foundry.utils.mergeObject(overrides, result);
    ...
    this.overrides = foundry.utils.mergeObject(this.overrides ?? {}, overrides);
    this.origins = foundry.utils.mergeObject(this.origins ?? {}, origins);
  }
  ...
  async modifyTokenAttribute(attribute, value, isDelta = false, isBar = true) {
    ...
    const current = foundry.utils.duplicate(getProperty(this.system, attribute));
    if (isBar) {
      if (attribute == "health") {
        const temp = foundry.utils.duplicate(getProperty(this.system, "tempHp"));
        if (isDelta) {
  ...
  async prepareDerivedData() {
    ...
    if (!isNaN(Number(actorSystem.stats.atk.mod))) {
      const stats = foundry.utils.duplicate(actorSystem.stats)
    ...
    // Update data structures.
    {
      ...
      if (!isNaN(actorSystem.skills.acrobatics.value)) {
        const skills = foundry.utils.duplicate(actorSystem.skills)
      ...
      if (isNaN(actorSystem.modifiers.initiative.value)) {
        const modifiers = foundry.utils.duplicate(actorSystem.modifiers);
    ...
    if (actorData.type === 'character' && actorSystem.skills.intimidation) {
      let skills = foundry.utils.duplicate(actorSystem.skills);
  ...
  _prepareCharacterData() {
    ...
    data.stats = CalculatePoisonedCondition(foundry.utils.duplicate(data.stats), actorData.flags?.ptu);
  ...
  async rollMove(move, options = { moveId, bonusDamage, targets, APBonus }) {
    ...
    if (moveData.frequency.toLowerCase().includes("daily") || moveData.frequency.toLowerCase().includes("scene")) {
      await move.update({ "system.useCount": Number(foundry.utils.duplicate(moveData.useCount ?? 0)) + 1 })
    }
    ...
    setTimeout(async () => {
      ...
      if (messageData.targetAmount >= 1 && attack.crit != CritOptions.CRIT_MISS) {
        const applicatorMessageData = foundry.utils.duplicate(messageData);
