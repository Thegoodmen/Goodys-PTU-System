...
export class PTUGen8CharacterSheet extends ActorSheet {
	...
	activateListeners(html) {
		...
		html.find('.effect-suspend').click(async (ev) => {
			...
			const effectId = button.dataset.id;
			const effect = this.actor.effects.get(effectId);
			const effectData = foundry.utils.duplicate(effect.data);
	...
	// 	item = foundry.utils.duplicate(await fromUuid(uuid));
	...
	// 		const quantity = foundry.utils.duplicate(oldItem.system.quantity);
	...
	async _onDexSort(event) {
		...
		for(const [index, entry] of entries.entries()) {
			const item = foundry.utils.duplicate(entry);
	...
	_onMoveRoll(event, { actor, item } = {}) {
		...
		let PerformFullAttack = (damageBonus = 0) => {
			const moveData = foundry.utils.duplicate(move.data);
...
async function sendMoveMessage(messageData = {}) {
	...
	Hooks.call("ptu.SendMoveToChat", foundry.utils.duplicate(messageData));
