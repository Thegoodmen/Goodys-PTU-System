...
async function ApplyChanges() {
    if(this.actor.update === undefined) {
        const actorData = foundry.utils.mergeObject(foundry.utils.mergeObject(this.actor, this.data), {system: foundry.utils.mergeObject(this.actor.system, this.system)});
    ...
    if(this.items.length > 0)
        await this.actor.createEmbeddedDocuments("Item", foundry.utils.duplicate(this.items));
    return this.actor;
}
...
function PrepareStats(type, randomPercent = 0.1) {
    let stats = {atk: {},def: {},spatk: {},spdef: {},spd: {},hp: {}};//foundry.utils.duplicate(this.actor.data.data.stats);
    let levelUpPoints = this.actor.system.levelUpPoints ? foundry.utils.duplicate(this.actor.system.levelUpPoints) : this.actor.system.level.current + 10;
