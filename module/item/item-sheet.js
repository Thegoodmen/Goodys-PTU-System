...
export async function sendItemMessage(messageData = {}) {
	...
	Hooks.call("ptu.SendItemToChat", foundry.utils.duplicate(messageData));
...
export async function useItem(event){
    ...
    if (actor.type == "character"){
        ...
        await item.update({"system.quantity": Number(foundry.utils.duplicate(item.system.quantity)) - 1});
    }
