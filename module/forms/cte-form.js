...
export class PTUCustomTypingEditor extends FormApplication {
    ...
    getData() {
      ...
      this.object = foundry.utils.isEmpty(this.object ?? {}) ? {type: "Normal", data: data.types.Normal} : this.object;
    ...
    activateListeners(html) {
      ...
      html.find('.item-create[data-type="type"]').click(async (ev) => {
        const newTypes = foundry.utils.duplicate(game.ptu.data.TypeEffectiveness);
      ...
      html.find('.delete-cte').click(async (ev) => {
        ...
        const newTypes = foundry.utils.duplicate(game.ptu.data.TypeEffectiveness);
        log(`Deleting type: ${this.object.type}. Data backup:`, foundry.utils.duplicate(this.object))
