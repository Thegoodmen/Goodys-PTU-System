...
export class PTUPokemonCharactermancer extends FormApplication {
  ...
  getData() {
    ...
    console.log(foundry.utils.duplicate(this));
  ...
  async _afterRender() {
    ...
    if(this.object.getFlag("ptu", "cmbackup")) {
      let current = foundry.utils.duplicate(this.store.state)
      let backup = foundry.utils.duplicate(this.object.getFlag("ptu", "cmbackup"));
  ...
  /** @override */
  async _updateObject() {
    const formData = foundry.utils.duplicate(this.store.state);
  ...
  async close(options) {
    ...
    if(options === undefined)
      await this.object.setFlag("ptu", "cmbackup", foundry.utils.duplicate(this.store.state));
    else
      await this.object.unsetFlag("ptu", "cmbackup");
  }
}