    ...
    async render() {
        ...
        if (this.updated <= 1) {
            ...
            if (this.state.moves.length > 0) {
                ...
                if (!foundry.utils.isEmpty(diffObject(this.state.moves, foundry.utils.duplicate(this.state.actor.itemTypes.move))) || !foundry.utils.isEmpty(diffObject(foundry.utils.duplicate(this.state.actor.itemTypes.move), this.state.moves))) {
                    this.updated += 1;
                    await this.store.dispatch("updateMoves", this.state.actor.itemTypes.move);
                    return;
                }
        ...
        for (const move of this.state.moves.sort(this._sort) ?? []) {
            ...
            const moveData = foundry.utils.duplicate(move);
