    ...
    async render() {
        ...
        if (this.updated <= 1) {
            ...
            if (this.state.abilities.length > 0) {
                ...
                if (!foundry.utils.isEmpty(diffObject(this.state.abilities, foundry.utils.duplicate(this.state.actor.itemTypes.ability))) || !foundry.utils.isEmpty(diffObject(foundry.utils.duplicate(this.state.actor.itemTypes.ability), this.state.abilities))) {
                    this.updated += 1;
                    await this.store.dispatch("updateAbilities", this.state.actor.itemTypes.ability);
                    return;
                }
        ...
        for (const ability of this.state.abilities?.sort(this._sort.bind(this)) ?? []) {
            ...
            const abilityData = foundry.utils.duplicate(ability);
