    ...
    async render() {
        ...
        if (this.updated <= 1) {
            ...
            if (this.state.features.length > 0) {
                ...
                if (!foundry.utils.isEmpty(diffObject(this.state.features, foundry.utils.duplicate(this.state.actor.itemTypes.feat))) || !foundry.utils.isEmpty(diffObject(foundry.utils.duplicate(this.state.actor.itemTypes.feat), this.state.features))) {
                    this.updated += 1;
                    await this.store.dispatch("updateFeatures", this.state.actor.itemTypes.feat);
                    return;
                }
        ...
        for (const feature of this.state.features?.sort(this._sort.bind(this)) ?? []) {
            ...
            const featureData = foundry.utils.duplicate(feature);
