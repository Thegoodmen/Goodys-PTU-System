...
export async function ThrowPokeball(thrower, target, pokeball) {
    ...
    pokeball.update({"system.quantity": Number(foundry.utils.duplicate(pokeball.system.quantity)) - 1});
        ...
        else // Escaped!
        {
            ...
            if (game.settings.get("ptu", "trackBrokenPokeballs"))
            {
                ...
                if (brokenBall) //if they do, increment the quantity
                    await brokenBall.update({"system.quantity": foundry.utils.duplicate(brokenBall.system.quantity)+1})
