...
export const EffectFns = new Map([
    ...
    ["paralyzed", async function (tokenId, combat, lastCombatant, roundData, options, sender, effect, isStartOfTurn) {
        ...
        if (roll.total >= (isErrata ? CONFIG.PTUCombat.DC.PARALYZED : CONFIG.PTUCombat.DC.PARALYZED_PRE_ERRATA)) {
            ...
            if (isErrata) {
                const aeAffliction = foundry.utils.mergeObject(CONFIG.statusEffects.find(x => x.id == "effect.other.vulnerable"), {
...
Hooks.on("applyActiveEffect", function (actorData, change) {
    if (change.key == "data.modifiers.flinch_count") {
        ...
        let count = foundry.utils.duplicate(actor.system).modifiers.flinch_count;
...
function applyPreCreateActiveEffectChanges(effect, preCreate = true) {
    ...
    if (game.combats.active) {
        data.duration = foundry.utils.mergeObject(effect.duration ?? {}, {
        ...
        else data.flags = foundry.utils.mergeObject(effect.flags ?? {}, { ptu: { roundsElapsed: 0 } });
    }
    else {
        data.duration = foundry.utils.mergeObject(effect.duration ?? {}, { startRound: -1, startTurn: -1 });
        if (preCreate) data["flags.ptu.roundsElapsed"] = 0;
        else data.flags = foundry.utils.mergeObject(effect.flags ?? {}, { ptu: { roundsElapsed: -1 } });
    }
    if(effect.flags?.core?.statusId.includes("effect.")) {
        data.flags = foundry.utils.mergeObject(data.flags ?? effect.flags, {"ptu.editLocked": true})
    }
    return data;
}